...
fetch(RSS_URL)
  ...
  .then(data => {
    ...
    items.forEach(function (update, index) {
      ...
      if (pubDate > oneWeekAgo) {
        ...
        if (updateCount <= maxUpdates) {
          ...
          updateDiv.innerHTML = `
            <div id="title${updateCount}" style="margin-top:16px; font-size:18px;"><a href="${link}" target="_blank" style="color: #ffffff; text-decoration: underline;">${title}</a> <span id="subtitle${updateCount}" style="font-size:12px;"> -- ${pubDate}</span></div>
    ...
    if (updateCount === 0) {
      ...
      document.getElementById("statusBanner").style.padding = boxPadding;
      let updateHTML = `<div id="noUpdates" style="padding:8px; font-size:18px;">There have been no updates in the last ${timeFrame} ${daysText}.</div>`;
